...
def thread_function() -> None:
    ...
    if main_thread != current_thread:
        print(f"Рабочий поток: {current_thread.name=}, {current_thread.daemon=}, {current_thread.ident=}")
    else:
        print(f"Главный поток: {main_thread.name=}, {main_thread.daemon=}, {main_thread.ident=}")
